...
export const courseSchema = z.object({
  ...
  price: z.coerce.number().min(1, { message: "Price must be at positive number" }),
  duration: z.coerce
    .number()
    .min(1, { message: "Duration must be at least 1 hour long" })
    .max(100, { message: "Duration cannot exceed 100 hours" }),
  level: z.enum(courseLevel, { message: "Level is required" }),
  category: z.enum(courseCategory, { message: "Category is required" }),
  smallDescription: z
    .string()
    .min(20, { message: "Small description must be at least 20 characters long" })
